...
void CinemaReservation::Initialize()
{
	...
	showList.AddShowtime(movieList.getMovieAtPosition(movieIndex), "Monday to Friday at 15h15", "17h27", hallsIndex);
	showList.AddShowtime(movieList.getMovieAtPosition(movieIndex), "Monday to Friday at 18h15", "20h27", hallsIndex);
	showList.AddShowtime(movieList.getMovieAtPosition(movieIndex), "Monday to Friday at 21h15", "23h27", hallsIndex);
	movieIndex++;
	...
	showList.AddShowtime(movieList.getMovieAtPosition(movieIndex), "Saturday, Sunday at 15h25", "18h20", hallsIndex);
	showList.AddShowtime(movieList.getMovieAtPosition(movieIndex), "Saturday, Sunday at 18h25", "21h20", hallsIndex);
	showList.AddShowtime(movieList.getMovieAtPosition(movieIndex), "Saturday, Sunday at 21h25", "0h20", hallsIndex);
	movieIndex++;
	...
	movieList.addMovie("Interstellar", 169, "PG-13", "A team of explorers travel through a wormhole in space in an attempt to ensure humanity's survival.", "Sci-Fi", "Christopher Nolan", "Matthew McConaughey", 8.6f);
	movieList.addMovie("Leon: The Professional", 110, "R", "Mathilda, a 12-year-old girl, is reluctantly taken in by Léon, a professional assassin, after her family is murdered. An unusual relationship forms as she becomes his protégée and learns the assassin's trade.", "Crime", "Luc Besson", "Jean Reno", 8.5f);
